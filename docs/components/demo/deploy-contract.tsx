...
function DeployContractInner() {
  ...
  const { isError, error, send, data, isPending } = useSendTransaction({
    calls: udc && address ? [udc.populate("deploy_contract", [classHash, 23, false, getCallData(address)])] : undefined,
  });
    ...
    <div className="flex flex-col gap-4">
      {chain.id === BigInt(constants.StarknetChainId.SN_SEPOLIA) ? (
        ...
        <>
          ...
          {switchChainError && (
            <p className="text-red-500">{switchChainError.message}, Try switching to Sepolia Manually!</p>
          )}
          <Button onClick={async () => await switchChainAsync()}>Switch to Sepolia</Button>
        </>
      )}
    </div>
  );
...
const classHash = "0x07f3777c99f3700505ea966676aac4a0d692c2a9f5e667f4c606b51ca1dd3420";
