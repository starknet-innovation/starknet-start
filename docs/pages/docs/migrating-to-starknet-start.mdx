...
This migration guide intentionally mentions `starknet-react`, `@starknet-react/core`, and Starknet React only as historical source-side references for projects upgrading to Starknet Start. Keep current-facing installation, import, API, and integration guidance branded as Starknet Start and using `@starknet-start/*` package names.
