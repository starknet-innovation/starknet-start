...
By default this hook fetches events in pages of 5 starting from genesis, but you can pass arguments for filtering.
...
```ts twoslash
import { useEvents } from "@starknetfoundation/starknet-start-react";
...
- Type: `InfiniteData<Events, string> | undefined`
...
- Type: `UseEventsResult["fetchNextPage"]`

`UseEventsResult` is the return type of the `useEvents` hook.
