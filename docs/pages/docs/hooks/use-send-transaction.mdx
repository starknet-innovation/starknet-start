...
```ts twoslash
import { useSendTransaction, useContract, useNetwork, useAccount } from "@starknetfoundation/starknet-start-react";
...
### Replacing `account.execute`

If you previously called `account.execute(calls)` from `useAccount`, create the
hook without default calls and pass the calls to `send` or `sendAsync`.
