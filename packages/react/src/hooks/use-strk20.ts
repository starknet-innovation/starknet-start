...
export type UseStrk20BalancesArgs = {
  /** Token addresses to query. Pass an empty array to return all shielded balances. */
  tokens?: Address[];
};
...
export function useStrk20InvokeTransaction(
  ...
  const invoke = (args?: STRK20_ACTION[]) => {
    return request(args ? { params: { actions: args }, type: "wallet_strk20InvokeTransaction" } : undefined);
  };

  const invokeAsync = (args?: STRK20_ACTION[]) => {
    return requestAsync(args ? { params: { actions: args }, type: "wallet_strk20InvokeTransaction" } : undefined);
  };
